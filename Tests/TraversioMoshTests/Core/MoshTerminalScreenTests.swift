...
struct MoshTerminalScreenTests {
    ...
    @Test
    func zeroCountCursorTabulationUsesDefaultCountAndPreservesPendingWrap() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 12, rows: 2))
        var forwardScreen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 2))
        var backwardScreen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 2))

        try screen.apply(MoshTerminalOutput(bytes: Array("\u{1b}[3gU\u{1b}[0IV\u{1b}[1;10HW\u{1b}[0ZX".utf8)))
        try forwardScreen.apply(MoshTerminalOutput(bytes: Array("abcd\u{1b}[0IX".utf8)))
        try backwardScreen.apply(MoshTerminalOutput(bytes: Array("abcd\u{1b}[0ZX".utf8)))

        #expect(screen.snapshot.lineStrings == ["X        W V", "            "])
        #expect(screen.snapshot.cursor == MoshTerminalCursor(row: 0, column: 1))
        #expect(forwardScreen.snapshot.lineStrings == ["abcd", "X   "])
        #expect(forwardScreen.snapshot.cursor == MoshTerminalCursor(row: 1, column: 1))
        #expect(backwardScreen.snapshot.lineStrings == ["abcd", "X   "])
        #expect(backwardScreen.snapshot.cursor == MoshTerminalCursor(row: 1, column: 1))
    }
    ...
    @Test
    func unsupportedDeviceStatusReportDoesNotReplyButClearsPendingWrap() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 2))

        let replies = try screen.apply(MoshTerminalOutput(bytes: Array("abcd\u{1b}[7nX".utf8)))
    ...
    @Test
    func c1CSIInsideOSCFinalizesMetadataAndStartsCSISequenceLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 5, rows: 1))

        try screen.apply(MoshTerminalOutput(bytes: Array("A\u{1b}]2;Nested Title\u{009b}2CB".utf8)))
    ...
    func c1NELInsideOSCFinalizesMetadataExecutesAndReturnsToGroundLikeOfficialMosh() throws {
        ...
        try screen.apply(MoshTerminalOutput(bytes: Array("A\u{1b}]2;Nested Title\u{0085}B".utf8)))
    ...
    @Test
    func osc8RejectsDeletePayloadWithoutChangingCurrentHyperlink() throws {
        let delete = "\u{7f}"
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 1))
        let link = MoshTerminalHyperlink(parameters: "id=1", url: "https://example.test")
                ...
                bytes: Array(
                    (
                        "\u{1b}]8;id=1;https://example.test\u{1b}\\A"
                            + "\u{1b}]8;id=2;https://example\(delete).test\u{1b}\\B"
                            + "\u{1b}]8;;\u{1b}\\C"
                    ).utf8
                )
            )
        ...
        #expect(screen.snapshot.rows[0][0].hyperlink == link)
        #expect(screen.snapshot.rows[0][1].hyperlink == link)
        #expect(screen.snapshot.rows[0][2].hyperlink == nil)
        #expect(screen.snapshot.currentHyperlink == nil)
    }
    ...
    func osc8InvalidPayloadLiveFixtureMatchesOfficialScreenState() throws {
        let delete = "\u{7f}"
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 40, rows: 6))
        let marker = "TERMINAL-SCREEN-HYPERLINK-INVALID-OK"
    ...
    @Test
    func c1NELInsideDCSExecutesAndReturnsToGroundLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 2))

        try screen.apply(MoshTerminalOutput(bytes: Array("A\u{1b}Pignored\u{0085}B".utf8)))
    ...
    @Test
    func zeroScrollRegionParametersResetToFullScreenLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 4))

        try screen.apply(MoshTerminalOutput(bytes: Array("1111222233334444\u{1b}[2;3r\u{1b}[0;0r\u{1b}[S".utf8)))

        #expect(screen.snapshot.lineStrings == ["2222", "3333", "4444", "    "])
        #expect(screen.snapshot.cursor == MoshTerminalCursor(row: 0, column: 0))
    }

    @Test
    func missingScrollRegionBottomDefaultsToScreenBottomLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 4))

        try screen.apply(MoshTerminalOutput(bytes: Array("1111222233334444\u{1b}[2r\u{1b}[4;1H\nX".utf8)))

        #expect(screen.snapshot.lineStrings == ["1111", "3333", "4444", "X   "])
    ...
    @Test
    func oversizedScrollRegionBottomClampsToScreenBottomLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 4))

        try screen.apply(MoshTerminalOutput(bytes: Array("1111222233334444\u{1b}[3;9r\u{1b}[4;1H\nX".utf8)))

        #expect(screen.snapshot.lineStrings == ["1111", "2222", "4444", "X   "])
        #expect(screen.snapshot.cursor == MoshTerminalCursor(row: 3, column: 1))
    }
    ...
    @Test
    func invertedScrollRegionIsIgnoredLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 4))

        try screen.apply(MoshTerminalOutput(bytes: Array("1111222233334444\u{1b}[2;3r\u{1b}[3;1H\u{1b}[3;2r\nX".utf8)))
    ...
    @Test
    func zeroOneScrollRegionIsIgnoredLikeOfficialMosh() throws {
        var screen = try MoshTerminalScreen(dimensions: MoshTerminalDimensions(columns: 4, rows: 4))

        try screen.apply(MoshTerminalOutput(bytes: Array("1111222233334444\u{1b}[2;3r\u{1b}[3;1H\u{1b}[0;1r\nX".utf8)))
